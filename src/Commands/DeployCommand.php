 ...
 * Class DeployCommand.
 *
 * This is a temporary copy of DeployCommand from Terminus core to remove one check that we don't need for eVCS sites.
 *
    ...
    public function deploy(
        ...
        if ($env->isInitialized()) {
            if (!$env->hasDeployableCode()) {
                // ICR sites will report nothing to deploy, still trigger the deployment as there may be commits to actually deploy.
                if ($site->getUpstream()->get('type') !== "icr") {
